...
local ClosureBindings = {
	...
	[22] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(22)local TweenService = game:GetService("TweenService")
      ...
      function Element:New(Idx, Config)
      	...
      	-- ระบบตรวจ "เลื่อนจริง" แบบเดียวกับ Maru Hub: ดูจากตำแหน่ง CanvasPosition ที่เปลี่ยนจริง
      	-- ไม่ใช่แค่ระยะนิ้วตั้งแต่กดจนปล่อยของแต่ละแถว (ซึ่งเฟรมแรกๆ ของการเลื่อนเร็วๆ
      	-- ระยะอาจยังไม่เกิน threshold ทำให้ติดเป็น "แตะ" ทั้งที่กำลังเลื่อนอยู่)
      	-- ถ้า Canvas เคลื่อนเกินเกณฑ์เล็กน้อยระหว่างที่นิ้วกดอยู่ ให้ทุกแถวรู้ว่ากำลังเลื่อน
      	-- และห้ามเลือกจนกว่าจะปล่อยนิ้วครั้งใหม่ (กันเลือกเองตอน flick เร็วๆ)
      	-- หมายเหตุ: ใช้ UserInputService ตรวจจังหวะกดนิ้ว/ปล่อยนิ้ว ไม่ใช่ InputBegan ของตัว
      	-- ScrollingFrame เอง เพราะปุ่มแต่ละแถวบังพื้นที่อยู่ ทำให้ ScrollingFrame ไม่ได้รับ
      	-- input ตรงๆ (GuiObject ไม่ bubble input จากลูกขึ้นมาให้ตัวแม่)
      	...
      	Creator.AddSignal(DropdownScrollFrame:GetPropertyChangedSignal("CanvasPosition"), function()
      		...
      		if DropdownScrollTouchActive and (Current - LastCanvasPosition).Magnitude > 2 then
      	...
      	Creator.AddSignal(UserInputService.InputEnded, function(Input)
      		if Input.UserInputType == Enum.UserInputType.Touch then
      			...
      			-- หน่วงรีเซ็ตเล็กน้อยกันแถวที่กำลังจะปล่อยนิ้วพร้อมกันยังเห็นว่ากำลังเลื่อนอยู่
    ...
    if #Dropdown.Values > 10 then
        ...
        DropdownHolderCanvas.Size = UDim2.fromOffset(ListSizeX, math.min(ContentHeight, MaxHeight))
      	...
      	function Dropdown:Open()
      		-- ปิด Dropdown อันอื่นที่เปิดอยู่ก่อนเสมอ กันไม่ให้แผงซ้อนกันหลายอัน
      		-- (ของเดิมไม่มีตัวนี้ ทำให้เปิดอันใหม่ทับอันเก่าที่ยังเปิดอยู่)
      		if Library.OpenDropdown and Library.OpenDropdown ~= Dropdown and Library.OpenDropdown.Close then
	...
	if SearchText ~= "" then
		if not string.find(string.lower(Value), SearchText, 1, true) then
			continue
      			...
      			local BtnTouchActive = false

      			-- เดิม Mouse.../Button1Down/Up ทำงานคู่ขนานกับตัวจับ touch ด้านล่าง
      			-- บนมือถือ Roblox จำลอง MouseEnter/MouseButton1Down ตอนแตะได้ แต่ตอนลากนิ้วผ่าน
      			-- (scroll) มักไม่ยิง MouseLeave/MouseButton1Up ให้คืนสีกลับ ทำให้ค้างเป็นสีเทา
      			-- จึงกันไว้ไม่ให้ชุด Mouse... ทำงานซ้ำตอนกำลังมี touch อยู่บนปุ่มนี้
      			Creator.AddSignal(Button.MouseEnter, function()
      				if BtnTouchActive then return end
      			...
      			Button.InputChanged:Connect(function(Input)
      				if Input.UserInputType == Enum.UserInputType.Touch and BtnTouchStart then
      					if (Input.Position - BtnTouchStart).Magnitude > 8 then
      						BtnTouchMoved = true
      						-- ลากนิ้วผ่าน (กำลัง scroll) ให้คืนสีกลับทันที ไม่ให้ค้างเป็นสีเทา
      						SetBackTransparency(Selected and 0.89 or 1)
      					end
      				end
      			end)

      			-- กันค้างชั้นสุดท้าย: ถ้านิ้วถูกปล่อยที่ไหนก็ตามบนจอ (รวมถึงหลุดออกนอกปุ่มนี้ระหว่าง
      			-- ลาก/scroll) ให้รีเซ็ตสถานะ touch ของปุ่มนี้และคืนสีกลับเสมอ ไม่ให้มีจังหวะใดค้างเทา
      			Creator.AddSignal(UserInputService.TouchEnded, function()
      				if BtnTouchActive then
      			...
      			-- เดิมมี ButtonLabel.InputBegan ดักจับ MouseButton1 ซ้ำกับ Button (ตัวเดียวกันซ้อนกัน)
      			-- บนมือถือทำให้เกิดการเลือกซ้ำ/สุ่ม และ MouseEnter/MouseLeave ไม่ reset ตอนลากนิ้วผ่าน
      			-- (ทำให้แถบชื่อค้างเป็นสีเทา) จึงลบออก ให้ Button (ทั้งแถว) เป็นจุดรับ input เดียว
	...
	[26] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(26)local UserInputService = game:GetService("UserInputService")
		...
		function Element:New(Idx, Config)
			...
			local function UpdateFromInputPosition(InputPosition)
				...
				local SizeScale = math.clamp((InputPosition.X - AbsPos.X) / AbsSize.X, 0, 1)
			...
			Creator.AddSignal(SliderDot.InputBegan, function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					Dragging = true -- จับที่จุดอยู่แล้ว ไม่ต้องกระโดดค่า
				end
			end)
		...
		function Element:New(Idx, Config)
			...
			function Toggle:SetValue(Value)
				...
				if Toggle.Value then
					...
					Creator.AddThemeObject(ToggleCircle, { ImageColor3 = "ToggleSlider" })
				end
				TweenService:Create(
					ToggleCircle,
...
[51] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(51)return {
	...
	ToggleToggled = Color3.fromRGB(255, 255, 255),
	SliderRail = Color3.fromRGB(72, 138, 182),
	DropdownFrame = Color3.fromRGB(120, 120, 120),
	DropdownHolder = Color3.fromRGB(35, 35, 35),
	DropdownBorder = Color3.fromRGB(25, 25, 25),

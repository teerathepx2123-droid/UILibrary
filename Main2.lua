...
Creator.AddSignal(game:GetService("UserInputService").InputChanged, function(input)
    if isScrolling and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        local delta = scrollStartPos - input.Position.Y
        scrollDistance = scrollDistance + math.abs(delta)
    end
end)

for _, button in pairs(Dropdown.Buttons) do
    local originalCallback = button.MouseButton1Click
    Creator.AddSignal(button.MouseButton1Click, function()
        if scrollDistance < 5 and (tick() - scrollStartTime) < 0.5 then
            originalCallback:Fire()
        end
    end)
end
,
	...
	[27] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(27)local TweenService = game:GetService("TweenService")
		...
		function Element:New(Idx, Config)
			...
			function Toggle:SetValue(Value)
				...
				Creator.OverrideTag(ToggleCircle, { ImageColor3 = Toggle.Value and Color3.fromRGB(255, 255, 255) or "ToggleSlider" })
				TweenService:Create(
					ToggleCircle,
					TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ Position = UDim2.new(0, Toggle.Value and 16 or 2, 0.5, -10) }
				):Play()
				TweenService:Create(
					ToggleSlider,
					TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),

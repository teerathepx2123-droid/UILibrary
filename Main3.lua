...
local ClosureBindings = {
	...
	[18] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(18)local Root = script.Parent
		...
		function Creator.AddThemeObject(Object, Properties)
			...
			local currentTheme = Themes[require(Root).Theme] or Themes["Midnight"] or Themes["Dark"]
			if currentTheme then
				for Property, ColorIdx in next, Properties do
					local v = currentTheme[ColorIdx]
					if v ~= nil then Object[Property] = v end
				end
			end
			return Object
		end
	...
	[22] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(22)local TweenService = game:GetService("TweenService")
      ...
      function Element:New(Idx, Config)
      	...
      	local _lastCanvasPos = DropdownScrollFrame.CanvasPosition
      	local SCROLL_THRESHOLD = 6
      	...
      	Creator.AddSignal(UserInputService.InputBegan, function(Input)
      		if Input.UserInputType == Enum.UserInputType.Touch and Dropdown.Opened then
      			...
      			_touchTotalDelta = 0
      			_lastCanvasPos = DropdownScrollFrame.CanvasPosition
      	...
      	Creator.AddSignal(DropdownScrollFrame:GetPropertyChangedSignal("CanvasPosition"), function()
      		...
      		if _touchActive and math.abs(Current.Y - _lastCanvasPos.Y) > 0.5 then
      			DropdownIsScrolling = true
      		end
      		_lastCanvasPos = Current
      	end)
      	...
      	local function RecalculateListPosition()
      		...
      		if posX + listW > vp.X - margin then posX = vp.X - listW - margin end
      		...
      		if posY + listH > vp.Y - margin then posY = vp.Y - listH - margin end
      	...
      	local function RecalculateListSize()
      		...
      		local h = math.max(80, math.min(contentH, maxH))
	...
	[47] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(47)local ThemeCache = {}
		local ThemeNames = {}
		...
		local ThemeRefs = {}
		for _, Theme in next, script:GetChildren() do
			ThemeRefs[Theme.Name] = Theme
		end

		local Themes = setmetatable({ Names = ThemeNames }, {
			__index = function(_, key)
				if ThemeCache[key] ~= nil then
					return ThemeCache[key]
				end
				local ref = ThemeRefs[key]
				if ref then
					local data = require(ref)
					ThemeCache[key] = data
					return data
				end
			end,

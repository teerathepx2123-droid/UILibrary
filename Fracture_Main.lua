			...
			if Library.Window and table.find(Library.Themes, Value) then
				...
				if Value == "Rainbow" then
					RainbowThread = task.spawn(function()
						local hue = 0
						while Library.Theme == "Rainbow" do
							hue = (hue + 0.002) % 1
							local accent = Color3.fromHSV(hue, 0.9, 1)
							local bg = Color3.fromHSV(hue, 0.3, 0.1)
			...
			do
				...
				local function startMomentum()
					...
					runConn = game:GetService("RunService").RenderStepped:Connect(function(dt)
						...
						local newY = math.clamp(scrollFrame.CanvasPosition.Y + velocity, 0, math.max(0, maxY))
				...
				scrollFrame.InputChanged:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch and isDragging and touchStart then
						...
						if dt > 0 then
							velocity = (lastPos - input.Position.Y) / dt
						end
						lastPos = input.Position.Y
						lastTime = now
						local delta = touchStart - input.Position.Y
						local maxY = scrollFrame.AbsoluteCanvasSize.Y - scrollFrame.AbsoluteSize.Y
						local newY = math.clamp(touchStartCanvas + delta, 0, math.max(0, maxY))
						scrollFrame.CanvasPosition = Vector2.new(0, newY)
					end
				end)

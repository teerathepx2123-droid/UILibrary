		...
		},
		{
			...
			},
			{
				...
				},
				{
					...
					{ 49, "ModuleScript", { "Aqua Tide" } },
					{ 50, "ModuleScript", { "Midnight" } },
					{ 51, "ModuleScript", { "Obsidian" } },
					{ 52, "ModuleScript", { "Ivory" } },
					{ 53, "ModuleScript", { "Sakura" } },
					{ 54, "ModuleScript", { "Blood Moon" } },
					...
					{ 68, "ModuleScript", { "Neon Blue" } },
					{ 69, "ModuleScript", { "Neon Green" } },
					{ 70, "ModuleScript", { "Neon Purple" } },
...
if game:GetService("CoreGui"):FindFirstChild('Fracture Hub') then
	game:GetService("CoreGui"):FindFirstChild('Fracture Hub'):Destroy()
end
...
local ClosureBindings = {
		...
		function Library:SetTheme(Value)
			...
			if Library.Window and table.find(Library.Themes, Value) then
				...
				if Value == "Rainbow" then
					local T = Themes["Rainbow"]
					if not T then return end
					RainbowThread = RunService.Heartbeat:Connect(function(dt)
						RainbowHue = (RainbowHue + dt * 0.08) % 1
						local accent = Color3.fromHSV(RainbowHue, 0.9, 1)
						local bg = Color3.fromHSV(RainbowHue, 0.3, 0.12)
						T.Accent = accent
						T.Tab = accent
						T.SliderRail = accent
						T.ToggleSlider = accent
						T.AcrylicMain = bg
						Creator.UpdateTheme()
					end)
				end
	...
	[14] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(14)local Root = script.Parent.Parent
		...
		function TabModule:New(Title, Icon, Parent)
			...
			do
				local scrollFrame = Tab.ContainerFrame
				local velocity = 0
				local lastPos = 0
				local lastTime = tick()
				local touchStart = nil
				local touchStartCanvas = 0
				...
				local function startMomentum()
					...
					runConn = RS.Heartbeat:Connect(function(dt)
						if isDragging then return end
						velocity = velocity * FRICTION
						if math.abs(velocity) < MIN_VEL then
							velocity = 0
							runConn:Disconnect()
							runConn = nil
							return
						...
						local newY = math.clamp(cur + velocity * (dt * 60), 0, maxY)
						if newY ~= cur then
				...
				scrollFrame.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						...
						touchStartCanvas = scrollFrame.CanvasPosition.Y
				...
				scrollFrame.InputChanged:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch and isDragging and touchStart then
						local now = tick()
						local dt = now - lastTime
						if dt > 0.001 then
							velocity = (lastPos - input.Position.Y) / dt * 0.016
						end
						lastPos = input.Position.Y
						lastTime = now
						local delta = touchStart - input.Position.Y
						local maxY = math.max(0, scrollFrame.AbsoluteCanvasSize.Y - scrollFrame.AbsoluteSize.Y)
						local newY = math.clamp(touchStartCanvas + delta, 0, maxY)
						scrollFrame.CanvasPosition = Vector2.new(0, newY)
					end
				end)
				...
				scrollFrame.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						isDragging = false
						touchStart = nil
						if math.abs(velocity) > MIN_VEL * 0.016 then
							startMomentum()
						end
					end
				end)
			...
			Creator.AddSignal(ContainerLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
				local prev = Tab.ContainerFrame.CanvasPosition
				Tab.ContainerFrame.CanvasSize = UDim2.new(0, 0, 0, ContainerLayout.AbsoluteContentSize.Y + 2)
				Tab.ContainerFrame.CanvasPosition = prev
			end)
	...
	[17] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(17)
		local UserInputService = game:GetService("UserInputService")
		local Mouse = game:GetService("Players").LocalPlayer:GetMouse()
		local Camera = game:GetService("Workspace").CurrentCamera
	...
	[18] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(18)local Root = script.Parent
		...
		function Creator.GetThemeProperty(Property)
			...
			return Themes["Midnight"][Property]
		end
		...
		function Creator.AddThemeObject(Object, Properties)
			...
			local currentTheme = Themes[require(Root).Theme]
			local fallback = Themes["Midnight"]
			for Property, ColorIdx in next, Properties do
				local v = (currentTheme and currentTheme[ColorIdx]) or fallback[ColorIdx]
				if v ~= nil then
					Object[Property] = v
				end
			end
		...
		function Creator.OverrideTag(Object, Properties)
			if Creator.Registry[Object] then
				...
				Creator.Registry[Object] = { Object = Object, Properties = Properties, Idx = 0 }
			end
			local currentTheme = Themes[require(Root).Theme]
			local fallback = Themes["Midnight"]
			for Property, ColorIdx in next, Properties do
			...
			local function CreateColorDialog()
				...
				Creator.AddSignal(SatVibMap.InputBegan, function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
						...
						local cx = math.clamp(Input.Position.X, MinX, MaxX)
						local cy = math.clamp(Input.Position.Y, MinY, MaxY)
						Sat = (cx - MinX) / (MaxX - MinX)
						Vib = 1 - (cy - MinY) / (MaxY - MinY)
						Display()
					end
				end)
				...
				Creator.AddSignal(UserInputService.InputChanged, function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
						...
						elseif DraggingHue then
							...
							local cy = math.clamp(Input.Position.Y, MinY, MaxY)
							Hue = (cy - MinY) / (MaxY - MinY)
							Display()
						elseif DraggingAlpha and Config.Transparency then
							local MinY = TransparencySlider.AbsolutePosition.Y
							local MaxY = MinY + TransparencySlider.AbsoluteSize.Y
							local cy = math.clamp(Input.Position.Y, MinY, MaxY)
							Transparency = 1 - (cy - MinY) / (MaxY - MinY)
							Display()
						end
					end
				end)
	...
	[22] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(22)local TweenService = game:GetService("TweenService")
      ...
      function Element:New(Idx, Config)
        ...
        DropdownScrollFrame.Size = UDim2.new(1, -5, 1, -50)
    ...
    local MaxHeight = Camera.ViewportSize.Y * 0.45
    if #Dropdown.Values > 10 then
        DropdownHolderCanvas.Size = UDim2.fromOffset(ListSizeX, math.min(392, MaxHeight))
    else
        ...
        DropdownHolderCanvas.Size = UDim2.fromOffset(ListSizeX, math.min(ContentHeight, MaxHeight))
      	...
      	Creator.AddSignal(DropdownHolderCanvas:GetPropertyChangedSignal("AbsoluteSize"), RecalculateListPosition)
      	...
      	Creator.AddSignal(UserInputService.InputBegan, function(Input)
      		...
      		then
      			...
      			local hPos = DropdownHolderCanvas.AbsolutePosition
      			local hSize = DropdownHolderCanvas.AbsoluteSize
      			local bPos = DropdownInner.AbsolutePosition
      			local bSize = DropdownInner.AbsoluteSize
      			local inHolder = px >= hPos.X and px <= hPos.X + hSize.X and py >= hPos.Y and py <= hPos.Y + hSize.Y
      			local inBtn = px >= bPos.X and px <= bPos.X + bSize.X and py >= bPos.Y and py <= bPos.Y + bSize.Y
		...
		local Values = Dropdown.Values
      	...
      	function Dropdown:BuildDropdownList()
      		...
      		for Idx, Value in next, Values do
	if SearchText ~= "" then
		if not string.find(string.lower(Value), SearchText, 1, true) then
			continue
		end
	end
      			local Table = {}
      			...
      			local BtnTouchMoved = false
      			local BtnTouchStartTime = 0
      			local BTN_SCROLL_THRESHOLD = 12
      			local BTN_HOLD_TIME = 0.3
      			...
      			Creator.AddSignal(Button.InputChanged, function(Input)
      				if Input.UserInputType == Enum.UserInputType.Touch and BtnTouchStart then
      					if (Input.Position - BtnTouchStart).Magnitude > BTN_SCROLL_THRESHOLD then
      						BtnTouchMoved = true
      					end
      				end
      			end)
      			...
      			Creator.AddSignal(Button.InputEnded, function(Input)
      				if Input.UserInputType == Enum.UserInputType.Touch then
      					local elapsed = tick() - BtnTouchStartTime
      					if not BtnTouchMoved and elapsed < BTN_HOLD_TIME then
	...
	[26] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(26)local UserInputService = game:GetService("UserInputService")
		...
		function Element:New(Idx, Config)
			...
			local function SliderUpdateFromX(x)
				local railPos = SliderRail.AbsolutePosition.X
				local railSize = SliderRail.AbsoluteSize.X
				if railSize <= 0 then return end
				local scale = math.clamp((x - railPos) / railSize, 0, 1)
			...
			Creator.AddSignal(SliderInner.InputBegan, function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					Dragging = true
					SliderUpdateFromX(Input.Position.X)
				end
			end)
			...
			Creator.AddSignal(SliderDisplay.FocusLost, function(enter)
				if enter then
					local v = tonumber(SliderDisplay.Text)
					if v then
						Slider:SetValue(v)
					end
				end
	...
	[54] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(54)return {
		...
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(120, 30, 30), Color3.fromRGB(60, 15, 15)),
		AcrylicNoise = 0.92,

		TitleBarLine = Color3.fromRGB(80, 40, 40),
		Tab = Color3.fromRGB(200, 100, 100),

		Element = Color3.fromRGB(30, 20, 20),
		ElementBorder = Color3.fromRGB(60, 30, 30),
		InElementBorder = Color3.fromRGB(70, 35, 35),
		ElementTransparency = 0.86,

		ToggleSlider = Color3.fromRGB(255, 75, 75),
		ToggleToggled = Color3.fromRGB(0, 0, 0),

		SliderRail = Color3.fromRGB(255, 75, 75),

		DropdownFrame = Color3.fromRGB(35, 22, 22),
		DropdownHolder = Color3.fromRGB(25, 15, 15),
		DropdownBorder = Color3.fromRGB(60, 30, 30),
		DropdownOption = Color3.fromRGB(30, 20, 20),

		Keybind = Color3.fromRGB(30, 20, 20),

		Input = Color3.fromRGB(30, 20, 20),
		InputFocused = Color3.fromRGB(20, 12, 12),
		InputIndicator = Color3.fromRGB(50, 30, 30),

		Dialog = Color3.fromRGB(22, 14, 14),
		DialogHolder = Color3.fromRGB(28, 18, 18),
		DialogHolderLine = Color3.fromRGB(15, 10, 10),
		DialogButton = Color3.fromRGB(40, 25, 25),
		DialogButtonBorder = Color3.fromRGB(80, 50, 50),
		DialogBorder = Color3.fromRGB(255, 75, 75),
		DialogInput = Color3.fromRGB(32, 20, 20),
		DialogInputLine = Color3.fromRGB(60, 40, 40),

		Text = Color3.fromRGB(255, 255, 255),
		SubText = Color3.fromRGB(180, 130, 130),
		Hover = Color3.fromRGB(35, 22, 22),
		HoverChange = 0.04,
		}
	end,
	[49] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(49)return {
	...
	[51] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(51)return {
		...
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(22, 22, 28), Color3.fromRGB(12, 12, 18)),
		AcrylicNoise = 0.95,

		TitleBarLine = Color3.fromRGB(45, 45, 60),
		Tab = Color3.fromRGB(100, 100, 120),

		Element = Color3.fromRGB(60, 60, 75),
		ElementBorder = Color3.fromRGB(28, 28, 38),
		InElementBorder = Color3.fromRGB(50, 50, 65),
		ElementTransparency = 0.80,

		ToggleSlider = Color3.fromRGB(75, 75, 95),
		ToggleToggled = Color3.fromRGB(255, 255, 255),

		SliderRail = Color3.fromRGB(100, 160, 220),

		DropdownFrame = Color3.fromRGB(95, 95, 115),
		DropdownHolder = Color3.fromRGB(22, 22, 30),
		DropdownBorder = Color3.fromRGB(38, 38, 52),
		DropdownOption = Color3.fromRGB(55, 55, 70),

		Keybind = Color3.fromRGB(75, 75, 95),

		Input = Color3.fromRGB(65, 65, 82),
		InputFocused = Color3.fromRGB(10, 10, 16),
		InputIndicator = Color3.fromRGB(110, 110, 135),

		Dialog = Color3.fromRGB(26, 26, 34),
		DialogHolder = Color3.fromRGB(18, 18, 25),
		DialogHolderLine = Color3.fromRGB(14, 14, 20),
		DialogButton = Color3.fromRGB(26, 26, 34),
		DialogButtonBorder = Color3.fromRGB(55, 55, 70),
		DialogBorder = Color3.fromRGB(45, 45, 60),
		DialogInput = Color3.fromRGB(32, 32, 42),
		DialogInputLine = Color3.fromRGB(110, 110, 135),

		Text = Color3.fromRGB(235, 235, 245),
		SubText = Color3.fromRGB(150, 150, 172),
		Hover = Color3.fromRGB(70, 70, 88),
		HoverChange = 0.06,
		}
	end,
	[52] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(52)return {
...
[62] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(62)return {
	Name = "Ocean Breeze",
	Accent = Color3.fromRGB(0, 170, 255),
	AcrylicMain = Color3.fromRGB(10, 14, 22),
	AcrylicBorder = Color3.fromRGB(5, 8, 15),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(10,14,22), Color3.fromRGB(6,10,18)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(0, 60, 100),
	Tab = Color3.fromRGB(0, 170, 255),
	Element = Color3.fromRGB(18, 28, 45),
	ElementBorder = Color3.fromRGB(0, 60, 100),
	InElementBorder = Color3.fromRGB(0, 40, 70),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(200, 230, 255),
	ToggleToggled = Color3.fromRGB(20, 40, 65),
	SliderRail = Color3.fromRGB(0, 120, 200),
	DropdownFrame = Color3.fromRGB(150, 200, 255),
	DropdownHolder = Color3.fromRGB(10, 18, 30),
	DropdownBorder = Color3.fromRGB(0, 50, 90),
	DropdownOption = Color3.fromRGB(20, 35, 58),
	Keybind = Color3.fromRGB(12, 20, 35),
	Input = Color3.fromRGB(12, 20, 35),
	InputFocused = Color3.fromRGB(5, 20, 45),
	InputIndicator = Color3.fromRGB(15, 28, 48),
	Dialog = Color3.fromRGB(12, 18, 30),
	DialogHolder = Color3.fromRGB(15, 22, 36),
	DialogHolderLine = Color3.fromRGB(8, 14, 24),
	DialogButton = Color3.fromRGB(20, 40, 65),
	DialogButtonBorder = Color3.fromRGB(0, 60, 100),
	DialogBorder = Color3.fromRGB(0, 150, 230),
	DialogInput = Color3.fromRGB(18, 32, 52),
	DialogInputLine = Color3.fromRGB(0, 50, 90),
...
[63] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(63)return {
	Name = "Amber Dusk",
	Accent = Color3.fromRGB(255, 185, 0),
	AcrylicMain = Color3.fromRGB(16, 13, 8),
	AcrylicBorder = Color3.fromRGB(10, 8, 4),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(16,13,8), Color3.fromRGB(12,10,5)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(80, 58, 0),
	Tab = Color3.fromRGB(255, 185, 0),
	Element = Color3.fromRGB(38, 30, 14),
	ElementBorder = Color3.fromRGB(80, 58, 0),
	InElementBorder = Color3.fromRGB(55, 40, 0),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(255, 230, 160),
	ToggleToggled = Color3.fromRGB(50, 38, 10),
	SliderRail = Color3.fromRGB(200, 140, 0),
	DropdownFrame = Color3.fromRGB(255, 210, 100),
	DropdownHolder = Color3.fromRGB(18, 14, 6),
	DropdownBorder = Color3.fromRGB(70, 50, 0),
	DropdownOption = Color3.fromRGB(42, 32, 12),
	Keybind = Color3.fromRGB(20, 16, 6),
	Input = Color3.fromRGB(20, 16, 6),
	InputFocused = Color3.fromRGB(30, 20, 5),
	InputIndicator = Color3.fromRGB(28, 22, 8),
	Dialog = Color3.fromRGB(20, 16, 7),
	DialogHolder = Color3.fromRGB(24, 19, 8),
	DialogHolderLine = Color3.fromRGB(14, 11, 4),
	DialogButton = Color3.fromRGB(50, 38, 10),
	DialogButtonBorder = Color3.fromRGB(80, 58, 0),
	DialogBorder = Color3.fromRGB(220, 160, 0),
	DialogInput = Color3.fromRGB(35, 27, 10),
	DialogInputLine = Color3.fromRGB(60, 44, 0),
	Text = Color3.fromRGB(255, 235, 180),
	SubText = Color3.fromRGB(180, 140, 60),
	Hover = Color3.fromRGB(35, 26, 8),
	HoverChange = 0.04,
}
end,
[64] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(64)return {
	Name = "Emerald Mist",
	Accent = Color3.fromRGB(0, 255, 100),
	AcrylicMain = Color3.fromRGB(8, 16, 10),
	AcrylicBorder = Color3.fromRGB(4, 10, 6),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(8,16,10), Color3.fromRGB(5,12,7)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(0, 80, 30),
	Tab = Color3.fromRGB(0, 255, 100),
	Element = Color3.fromRGB(12, 30, 18),
	ElementBorder = Color3.fromRGB(0, 80, 35),
	InElementBorder = Color3.fromRGB(0, 55, 22),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(180, 255, 210),
	ToggleToggled = Color3.fromRGB(15, 45, 25),
	SliderRail = Color3.fromRGB(0, 180, 70),
	DropdownFrame = Color3.fromRGB(100, 255, 160),
	DropdownHolder = Color3.fromRGB(8, 20, 12),
	DropdownBorder = Color3.fromRGB(0, 70, 28),
	DropdownOption = Color3.fromRGB(14, 36, 22),
	Keybind = Color3.fromRGB(10, 22, 14),
	Input = Color3.fromRGB(10, 22, 14),
	InputFocused = Color3.fromRGB(8, 28, 15),
	InputIndicator = Color3.fromRGB(12, 28, 18),
	Dialog = Color3.fromRGB(10, 20, 13),
	DialogHolder = Color3.fromRGB(12, 24, 16),
	DialogHolderLine = Color3.fromRGB(6, 14, 9),
	DialogButton = Color3.fromRGB(16, 45, 26),
	DialogButtonBorder = Color3.fromRGB(0, 80, 35),
	DialogBorder = Color3.fromRGB(0, 220, 85),
	DialogInput = Color3.fromRGB(14, 34, 20),
	DialogInputLine = Color3.fromRGB(0, 60, 25),
	Text = Color3.fromRGB(190, 255, 215),
	SubText = Color3.fromRGB(80, 180, 110),
	Hover = Color3.fromRGB(12, 35, 20),
	HoverChange = 0.04,
}
end,
[65] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(65)return {
	Name = "Violet Dream",
	Accent = Color3.fromRGB(160, 60, 255),
	AcrylicMain = Color3.fromRGB(14, 10, 22),
	AcrylicBorder = Color3.fromRGB(8, 5, 14),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(14,10,22), Color3.fromRGB(10,7,18)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(60, 20, 110),
	Tab = Color3.fromRGB(160, 60, 255),
	Element = Color3.fromRGB(28, 18, 50),
	ElementBorder = Color3.fromRGB(65, 22, 120),
	InElementBorder = Color3.fromRGB(45, 15, 85),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(220, 190, 255),
	ToggleToggled = Color3.fromRGB(40, 22, 72),
	SliderRail = Color3.fromRGB(120, 40, 200),
	DropdownFrame = Color3.fromRGB(190, 140, 255),
	DropdownHolder = Color3.fromRGB(16, 10, 28),
	DropdownBorder = Color3.fromRGB(55, 18, 100),
	DropdownOption = Color3.fromRGB(32, 20, 58),
	Keybind = Color3.fromRGB(18, 12, 32),
	Input = Color3.fromRGB(18, 12, 32),
	InputFocused = Color3.fromRGB(22, 10, 40),
	InputIndicator = Color3.fromRGB(24, 15, 42),
	Dialog = Color3.fromRGB(18, 12, 30),
	DialogHolder = Color3.fromRGB(22, 15, 36),
	DialogHolderLine = Color3.fromRGB(12, 8, 22),
	DialogButton = Color3.fromRGB(40, 22, 72),
	DialogButtonBorder = Color3.fromRGB(65, 22, 120),
	DialogBorder = Color3.fromRGB(140, 50, 230),
	DialogInput = Color3.fromRGB(30, 18, 54),
	DialogInputLine = Color3.fromRGB(50, 16, 95),
	Text = Color3.fromRGB(230, 210, 255),
	SubText = Color3.fromRGB(140, 100, 210),
	Hover = Color3.fromRGB(30, 18, 55),
	HoverChange = 0.04,
}
end,
[66] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(66)return {
	Name = "Sunset Glow",
	Accent = Color3.fromRGB(255, 100, 0),
	AcrylicMain = Color3.fromRGB(18, 10, 5),
	AcrylicBorder = Color3.fromRGB(12, 6, 2),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(18,10,5), Color3.fromRGB(14,7,3)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(100, 40, 0),
	Tab = Color3.fromRGB(255, 100, 0),
	Element = Color3.fromRGB(42, 20, 8),
	ElementBorder = Color3.fromRGB(100, 40, 0),
	InElementBorder = Color3.fromRGB(70, 28, 0),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(255, 210, 170),
	ToggleToggled = Color3.fromRGB(55, 25, 8),
	SliderRail = Color3.fromRGB(210, 70, 0),
	DropdownFrame = Color3.fromRGB(255, 160, 80),
	DropdownHolder = Color3.fromRGB(20, 10, 4),
	DropdownBorder = Color3.fromRGB(85, 34, 0),
	DropdownOption = Color3.fromRGB(48, 22, 7),
	Keybind = Color3.fromRGB(22, 11, 4),
	Input = Color3.fromRGB(22, 11, 4),
	InputFocused = Color3.fromRGB(30, 12, 3),
	InputIndicator = Color3.fromRGB(28, 14, 5),
	Dialog = Color3.fromRGB(22, 11, 5),
	DialogHolder = Color3.fromRGB(26, 14, 6),
	DialogHolderLine = Color3.fromRGB(15, 7, 2),
	DialogButton = Color3.fromRGB(55, 25, 8),
	DialogButtonBorder = Color3.fromRGB(100, 40, 0),
	DialogBorder = Color3.fromRGB(230, 85, 0),
	DialogInput = Color3.fromRGB(38, 18, 6),
	DialogInputLine = Color3.fromRGB(75, 30, 0),
	Text = Color3.fromRGB(255, 225, 195),
	SubText = Color3.fromRGB(200, 130, 70),
	Hover = Color3.fromRGB(40, 18, 6),
	HoverChange = 0.04,
}
end,
[67] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(67)return {
	Name = "Arctic Frost",
	Accent = Color3.fromRGB(130, 220, 255),
	AcrylicMain = Color3.fromRGB(12, 16, 20),
	AcrylicBorder = Color3.fromRGB(7, 10, 13),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(12,16,20), Color3.fromRGB(8,12,16)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(40, 70, 90),
	Tab = Color3.fromRGB(130, 220, 255),
	Element = Color3.fromRGB(20, 30, 40),
	ElementBorder = Color3.fromRGB(40, 75, 100),
	InElementBorder = Color3.fromRGB(28, 55, 75),
	ElementTransparency = 0.85,
	ToggleSlider = Color3.fromRGB(210, 240, 255),
	ToggleToggled = Color3.fromRGB(25, 45, 60),
	SliderRail = Color3.fromRGB(80, 170, 215),
	DropdownFrame = Color3.fromRGB(170, 225, 255),
	DropdownHolder = Color3.fromRGB(13, 20, 28),
	DropdownBorder = Color3.fromRGB(35, 65, 88),
	DropdownOption = Color3.fromRGB(22, 36, 50),
	Keybind = Color3.fromRGB(15, 23, 32),
	Input = Color3.fromRGB(15, 23, 32),
	InputFocused = Color3.fromRGB(12, 25, 38),
	InputIndicator = Color3.fromRGB(18, 30, 42),
	Dialog = Color3.fromRGB(14, 22, 30),
	DialogHolder = Color3.fromRGB(17, 26, 36),
	DialogHolderLine = Color3.fromRGB(9, 14, 20),
	DialogButton = Color3.fromRGB(25, 45, 62),
	DialogButtonBorder = Color3.fromRGB(40, 75, 100),
	DialogBorder = Color3.fromRGB(110, 200, 240),
	DialogInput = Color3.fromRGB(20, 34, 48),
	DialogInputLine = Color3.fromRGB(32, 60, 82),
	Text = Color3.fromRGB(210, 240, 255),
	SubText = Color3.fromRGB(110, 170, 210),
	Hover = Color3.fromRGB(18, 32, 46),
	HoverChange = 0.04,
}
end,
[68] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(68)return {
	Name = "Neon Blue",
	Accent = Color3.fromRGB(0, 200, 255),
	AcrylicMain = Color3.fromRGB(5, 5, 15),
	AcrylicBorder = Color3.fromRGB(0, 80, 140),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(5,5,20), Color3.fromRGB(3,3,12)),
	AcrylicNoise = 0.99,
	TitleBarLine = Color3.fromRGB(0, 100, 180),
	Tab = Color3.fromRGB(0, 200, 255),
	Element = Color3.fromRGB(8, 15, 30),
	ElementBorder = Color3.fromRGB(0, 100, 180),
	InElementBorder = Color3.fromRGB(0, 70, 130),
	ElementTransparency = 0.88,
	ToggleSlider = Color3.fromRGB(0, 180, 255),
	ToggleToggled = Color3.fromRGB(0, 5, 20),
	SliderRail = Color3.fromRGB(0, 150, 230),
	DropdownFrame = Color3.fromRGB(0, 180, 255),
	DropdownHolder = Color3.fromRGB(5, 10, 22),
	DropdownBorder = Color3.fromRGB(0, 90, 160),
	DropdownOption = Color3.fromRGB(10, 20, 40),
	Keybind = Color3.fromRGB(8, 15, 30),
	Input = Color3.fromRGB(8, 15, 30),
	InputFocused = Color3.fromRGB(0, 10, 30),
	InputIndicator = Color3.fromRGB(0, 120, 200),
	Dialog = Color3.fromRGB(6, 12, 25),
	DialogHolder = Color3.fromRGB(8, 15, 30),
	DialogHolderLine = Color3.fromRGB(0, 60, 110),
	DialogButton = Color3.fromRGB(10, 22, 45),
	DialogButtonBorder = Color3.fromRGB(0, 100, 180),
	DialogBorder = Color3.fromRGB(0, 200, 255),
	DialogInput = Color3.fromRGB(10, 18, 38),
	DialogInputLine = Color3.fromRGB(0, 80, 150),
	Text = Color3.fromRGB(180, 240, 255),
	SubText = Color3.fromRGB(80, 160, 210),
	Hover = Color3.fromRGB(10, 22, 45),
	HoverChange = 0.04,
}
end,
[69] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(69)return {
	Name = "Neon Green",
	Accent = Color3.fromRGB(0, 255, 80),
	AcrylicMain = Color3.fromRGB(5, 12, 5),
	AcrylicBorder = Color3.fromRGB(0, 80, 30),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(5,12,5), Color3.fromRGB(3,8,3)),
	AcrylicNoise = 0.99,
	TitleBarLine = Color3.fromRGB(0, 100, 35),
	Tab = Color3.fromRGB(0, 255, 80),
	Element = Color3.fromRGB(8, 20, 10),
	ElementBorder = Color3.fromRGB(0, 110, 40),
	InElementBorder = Color3.fromRGB(0, 75, 28),
	ElementTransparency = 0.88,
	ToggleSlider = Color3.fromRGB(0, 220, 70),
	ToggleToggled = Color3.fromRGB(0, 8, 3),
	SliderRail = Color3.fromRGB(0, 180, 60),
	DropdownFrame = Color3.fromRGB(0, 220, 70),
	DropdownHolder = Color3.fromRGB(5, 14, 6),
	DropdownBorder = Color3.fromRGB(0, 90, 32),
	DropdownOption = Color3.fromRGB(10, 25, 12),
	Keybind = Color3.fromRGB(8, 20, 10),
	Input = Color3.fromRGB(8, 20, 10),
	InputFocused = Color3.fromRGB(3, 14, 5),
	InputIndicator = Color3.fromRGB(0, 130, 45),
	Dialog = Color3.fromRGB(6, 16, 7),
	DialogHolder = Color3.fromRGB(8, 20, 9),
	DialogHolderLine = Color3.fromRGB(0, 65, 22),
	DialogButton = Color3.fromRGB(10, 28, 12),
	DialogButtonBorder = Color3.fromRGB(0, 110, 40),
	DialogBorder = Color3.fromRGB(0, 255, 80),
	DialogInput = Color3.fromRGB(10, 22, 11),
	DialogInputLine = Color3.fromRGB(0, 85, 30),
	Text = Color3.fromRGB(180, 255, 200),
	SubText = Color3.fromRGB(80, 200, 110),
	Hover = Color3.fromRGB(10, 28, 12),
	HoverChange = 0.04,
}
end,
[70] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(70)return {
	...
	AcrylicMain = Color3.fromRGB(10, 5, 18),
	AcrylicBorder = Color3.fromRGB(70, 0, 120),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(10,5,18), Color3.fromRGB(7,3,14)),
	AcrylicNoise = 0.99,
	TitleBarLine = Color3.fromRGB(90, 0, 150),
	Tab = Color3.fromRGB(180, 0, 255),
	Element = Color3.fromRGB(18, 8, 32),
	ElementBorder = Color3.fromRGB(90, 0, 150),
	InElementBorder = Color3.fromRGB(65, 0, 110),
	ElementTransparency = 0.88,
	ToggleSlider = Color3.fromRGB(160, 0, 230),
	ToggleToggled = Color3.fromRGB(8, 0, 18),
	SliderRail = Color3.fromRGB(140, 0, 200),
	DropdownFrame = Color3.fromRGB(160, 0, 230),
	DropdownHolder = Color3.fromRGB(10, 5, 20),
	DropdownBorder = Color3.fromRGB(80, 0, 130),
	DropdownOption = Color3.fromRGB(20, 8, 38),
	Keybind = Color3.fromRGB(14, 6, 26),
	Input = Color3.fromRGB(14, 6, 26),
	InputFocused = Color3.fromRGB(8, 2, 18),
	InputIndicator = Color3.fromRGB(100, 0, 170),
	Dialog = Color3.fromRGB(12, 5, 22),
	DialogHolder = Color3.fromRGB(15, 7, 28),
	DialogHolderLine = Color3.fromRGB(60, 0, 100),
	DialogButton = Color3.fromRGB(20, 8, 38),
	DialogButtonBorder = Color3.fromRGB(90, 0, 150),
	DialogBorder = Color3.fromRGB(180, 0, 255),
	DialogInput = Color3.fromRGB(16, 6, 30),
	DialogInputLine = Color3.fromRGB(70, 0, 120),
	Text = Color3.fromRGB(230, 180, 255),
	SubText = Color3.fromRGB(150, 80, 220),
	Hover = Color3.fromRGB(20, 8, 40),
	HoverChange = 0.04,
}
end,
[71] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(71)return {
	...
	AcrylicMain = Color3.fromRGB(16, 5, 12),
	AcrylicBorder = Color3.fromRGB(120, 0, 80),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(16,5,12), Color3.fromRGB(10,3,8)),
	AcrylicNoise = 0.99,
	TitleBarLine = Color3.fromRGB(140, 0, 95),
	Tab = Color3.fromRGB(255, 0, 180),
	Element = Color3.fromRGB(28, 8, 20),
	ElementBorder = Color3.fromRGB(140, 0, 95),
	InElementBorder = Color3.fromRGB(100, 0, 68),
	ElementTransparency = 0.88,
	ToggleSlider = Color3.fromRGB(230, 0, 160),
	ToggleToggled = Color3.fromRGB(12, 2, 8),
	SliderRail = Color3.fromRGB(200, 0, 140),
	DropdownFrame = Color3.fromRGB(230, 0, 160),
	DropdownHolder = Color3.fromRGB(14, 4, 10),
	DropdownBorder = Color3.fromRGB(120, 0, 82),
	DropdownOption = Color3.fromRGB(30, 8, 22),
	Keybind = Color3.fromRGB(20, 6, 15),
	Input = Color3.fromRGB(20, 6, 15),
	InputFocused = Color3.fromRGB(12, 3, 9),
	InputIndicator = Color3.fromRGB(150, 0, 105),
	Dialog = Color3.fromRGB(16, 5, 12),
	DialogHolder = Color3.fromRGB(20, 6, 15),
	DialogHolderLine = Color3.fromRGB(90, 0, 62),
	DialogButton = Color3.fromRGB(30, 8, 22),
	DialogButtonBorder = Color3.fromRGB(140, 0, 95),
	DialogBorder = Color3.fromRGB(255, 0, 180),
	DialogInput = Color3.fromRGB(24, 7, 17),
	DialogInputLine = Color3.fromRGB(110, 0, 76),
	Text = Color3.fromRGB(255, 200, 235),
	SubText = Color3.fromRGB(210, 100, 170),
	Hover = Color3.fromRGB(32, 9, 24),
	HoverChange = 0.04,
}
end,
[72] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(72)return {
	...
	AcrylicMain = Color3.fromRGB(14, 7, 2),
	AcrylicBorder = Color3.fromRGB(120, 50, 0),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(14,7,2), Color3.fromRGB(10,5,1)),
	AcrylicNoise = 0.99,
	TitleBarLine = Color3.fromRGB(140, 58, 0),
	Tab = Color3.fromRGB(255, 120, 0),
	Element = Color3.fromRGB(26, 12, 4),
	ElementBorder = Color3.fromRGB(140, 58, 0),
	InElementBorder = Color3.fromRGB(100, 40, 0),
	ElementTransparency = 0.88,
	ToggleSlider = Color3.fromRGB(230, 100, 0),
	ToggleToggled = Color3.fromRGB(10, 4, 1),
	SliderRail = Color3.fromRGB(200, 85, 0),
	DropdownFrame = Color3.fromRGB(230, 100, 0),
	DropdownHolder = Color3.fromRGB(12, 6, 2),
	DropdownBorder = Color3.fromRGB(120, 50, 0),
	DropdownOption = Color3.fromRGB(28, 12, 4),
	Keybind = Color3.fromRGB(18, 8, 2),
	Input = Color3.fromRGB(18, 8, 2),
	InputFocused = Color3.fromRGB(10, 4, 1),
	InputIndicator = Color3.fromRGB(150, 62, 0),
	Dialog = Color3.fromRGB(14, 7, 2),
	DialogHolder = Color3.fromRGB(18, 8, 3),
	DialogHolderLine = Color3.fromRGB(90, 38, 0),
	DialogButton = Color3.fromRGB(28, 12, 4),
	DialogButtonBorder = Color3.fromRGB(140, 58, 0),
	DialogBorder = Color3.fromRGB(255, 120, 0),
	DialogInput = Color3.fromRGB(22, 10, 3),
	DialogInputLine = Color3.fromRGB(110, 46, 0),
	Text = Color3.fromRGB(255, 220, 180),
	SubText = Color3.fromRGB(210, 140, 70),
	Hover = Color3.fromRGB(30, 13, 4),
	HoverChange = 0.04,
}
end,
[73] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(73)return {
	...
	AcrylicNoise = 0.97,
	TitleBarLine = Color3.fromRGB(38, 38, 38),
	Tab = Color3.fromRGB(90, 90, 90),
	...
	DropdownFrame = Color3.fromRGB(70, 70, 70),
	...
	DialogHolderLine = Color3.fromRGB(8, 8, 8),
...
[74] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(74)return {
	Name = "Neon Blue",
	Accent = Color3.fromRGB(0, 200, 255),
	AcrylicMain = Color3.fromRGB(5, 10, 20),
	AcrylicBorder = Color3.fromRGB(0, 80, 140),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(5, 12, 22), Color3.fromRGB(3, 7, 15)),
	AcrylicNoise = 0.98,
	TitleBarLine = Color3.fromRGB(0, 90, 160),
	Tab = Color3.fromRGB(0, 180, 240),
	Element = Color3.fromRGB(8, 20, 38),
	ElementBorder = Color3.fromRGB(0, 60, 110),
	InElementBorder = Color3.fromRGB(0, 80, 140),
	ElementTransparency = 0.86,
	ToggleSlider = Color3.fromRGB(0, 140, 200),
	ToggleToggled = Color3.fromRGB(0, 10, 22),
	SliderRail = Color3.fromRGB(0, 200, 255),
	DropdownFrame = Color3.fromRGB(0, 160, 220),
	DropdownHolder = Color3.fromRGB(5, 12, 24),
	DropdownBorder = Color3.fromRGB(0, 70, 120),
	DropdownOption = Color3.fromRGB(8, 20, 40),
	Keybind = Color3.fromRGB(8, 18, 36),
	Input = Color3.fromRGB(8, 18, 36),
	InputFocused = Color3.fromRGB(3, 8, 18),
	InputIndicator = Color3.fromRGB(0, 120, 180),
	Dialog = Color3.fromRGB(5, 12, 24),
	DialogHolder = Color3.fromRGB(4, 10, 20),
	DialogHolderLine = Color3.fromRGB(0, 55, 100),
	DialogButton = Color3.fromRGB(8, 18, 36),
	DialogButtonBorder = Color3.fromRGB(0, 90, 150),
	DialogBorder = Color3.fromRGB(0, 200, 255),
	DialogInput = Color3.fromRGB(6, 15, 30),
	DialogInputLine = Color3.fromRGB(0, 100, 160),
	Text = Color3.fromRGB(180, 240, 255),
	SubText = Color3.fromRGB(80, 180, 220),
	Hover = Color3.fromRGB(8, 22, 44),
	HoverChange = 0.04,
}
end
} 
